      ...
      children: [
        Row(
          ...
          children: [
            ...
            AppBadge(
              label: status,
              tone: tone,
              icon: tone == AppBadgeTone.warning ? Icons.priority_high : null,
            ),
          ],
        ),
        ...
        Wrap(
          ...
          children: [
            ...
            if (category != null)
              _TaskMeta(icon: Icons.folder_outlined, label: category!),
            if (offline) const Icon(Icons.cloud_off_outlined, size: 16),
            if (assigneeInitials != null)
              AppAvatar(initials: assigneeInitials!, size: 24),
          ],
        ),
        if (progress != null) ...[
          const SizedBox(height: AppSpacing.small),
...
class AppPrimaryScrollView extends StatelessWidget {
  const AppPrimaryScrollView({
    super.key,
    required this.children,
    this.navigationClearance = true,
  });

...
final class DemoDataStore {
  DemoDataStore({AppClock clock = const SystemAppClock()})
    : now = clock.now().toUtc() {
    _build();
  }
  ...
  void _build() {
    organizations.add(
      const Organization(
        ...
        nameAr: 'المنظمة التجريبية',
        nameEn: 'Demo Organization',
      ),
    );
    roles.addAll(const [
      Role(
        id: DemoSeedIds.roleEmployee,
        code: SystemRoleCode.employee,
        nameAr: 'موظف',
        nameEn: 'Employee',
      ),
      Role(
        id: DemoSeedIds.roleManager,
        code: SystemRoleCode.manager,
        nameAr: 'مدير',
        nameEn: 'Manager',
      ),
      Role(
        id: DemoSeedIds.roleSenior,
        code: SystemRoleCode.seniorManagement,
        nameAr: 'إدارة عليا',
        nameEn: 'Senior Management',
      ),
      Role(
        id: DemoSeedIds.roleAdministrator,
        code: SystemRoleCode.administrator,
        nameAr: 'مسؤول',
    ...
    departments.addAll(const [
      Department(
        id: DemoSeedIds.executive,
        code: 'EXEC',
        nameAr: 'الإدارة التنفيذية',
        nameEn: 'Executive',
        managerUserId: DemoSeedIds.omar,
      ),
      Department(
        id: DemoSeedIds.operations,
        code: 'OPS',
        nameAr: 'العمليات',
        nameEn: 'Operations',
        parentDepartmentId: DemoSeedIds.executive,
        managerUserId: DemoSeedIds.sara,
      ),
      Department(
        id: DemoSeedIds.finance,
        code: 'FIN',
        nameAr: 'المالية',
        nameEn: 'Finance',
        parentDepartmentId: DemoSeedIds.executive,
      ...
      Department(
        id: DemoSeedIds.informationTechnology,
        code: 'IT',
        nameAr: 'تقنية المعلومات',
        nameEn: 'Information Technology',
        parentDepartmentId: DemoSeedIds.executive,
        managerUserId: DemoSeedIds.laila,
      ),
    ]);
    users.addAll(const [
      OrganizationUser(
        id: DemoSeedIds.ahmed,
        employeeNumber: 'E001',
        nameAr: 'أحمد حسن',
        nameEn: 'Ahmed Hassan',
        departmentId: DemoSeedIds.operations,
      ...
      OrganizationUser(
        ...
        nameEn: 'Sara Mahmoud',
        departmentId: DemoSeedIds.operations,
        managerId: DemoSeedIds.omar,
        roleId: DemoSeedIds.roleManager,
        status: UserStatus.active,
      ),
      OrganizationUser(
        id: DemoSeedIds.omar,
        employeeNumber: 'X001',
        nameAr: 'عمر النعيمي',
        nameEn: 'Omar Al Nuaimi',
        departmentId: DemoSeedIds.executive,
        roleId: DemoSeedIds.roleSenior,
        status: UserStatus.active,
      ),
      OrganizationUser(
        id: DemoSeedIds.laila,
        employeeNumber: 'A001',
        nameAr: 'ليلى يوسف',
        nameEn: 'Laila Youssef',
        departmentId: DemoSeedIds.informationTechnology,
        managerId: DemoSeedIds.omar,
        roleId: DemoSeedIds.roleAdministrator,
        status: UserStatus.active,
      ),
      OrganizationUser(
        id: DemoSeedIds.khaled,
        employeeNumber: 'E002',
        nameAr: 'خالد إبراهيم',
        nameEn: 'Khaled Ibrahim',
        departmentId: DemoSeedIds.informationTechnology,
        managerId: DemoSeedIds.laila,
        roleId: DemoSeedIds.roleEmployee,
        status: UserStatus.active,
      ),
      OrganizationUser(
        id: 'user-mariam-saleh',
        employeeNumber: 'E003',
        nameAr: 'مريم صالح',
        nameEn: 'Mariam Saleh',
        departmentId: DemoSeedIds.operations,
    ...
    teams.addAll(const [
      Team(
        id: DemoSeedIds.fieldOperations,
        code: 'FIELD',
        departmentId: DemoSeedIds.operations,
      ...
      Team(
        id: DemoSeedIds.serviceCoordination,
        code: 'SERVICE',
        departmentId: DemoSeedIds.operations,
        nameAr: 'تنسيق الخدمات',
        nameEn: 'Service Coordination',
      ),
      Team(
        id: DemoSeedIds.accountsPayable,
        code: 'AP',
        departmentId: DemoSeedIds.finance,
        nameAr: 'الحسابات الدائنة',
        nameEn: 'Accounts Payable',
      ),
      Team(
        id: DemoSeedIds.reporting,
        code: 'REPORT',
        departmentId: DemoSeedIds.finance,
        nameAr: 'التقارير',
        nameEn: 'Reporting',
      ...
      Team(
        id: DemoSeedIds.recruitment,
        code: 'RECRUIT',
        departmentId: DemoSeedIds.humanResources,
        nameAr: 'التوظيف',
        nameEn: 'Recruitment',
      ),
      Team(
        id: DemoSeedIds.employeeServices,
        code: 'SERVICES',
        departmentId: DemoSeedIds.humanResources,
      ...
      Team(
        id: DemoSeedIds.technicalSupportQueue,
        code: 'SUPPORT',
        departmentId: DemoSeedIds.informationTechnology,
        nameAr: 'الدعم الفني',
        nameEn: 'Technical Support Queue',
        managerUserId: DemoSeedIds.laila,
        isQueueEnabled: true,
      ),
      Team(
        id: DemoSeedIds.applicationSupport,
        code: 'APPS',
        departmentId: DemoSeedIds.informationTechnology,
        nameAr: 'دعم التطبيقات',
        nameEn: 'Application Support',
      ),
    ]);
    memberships.addAll([
      _membership(
        'membership-ahmed',
        DemoSeedIds.fieldOperations,
        DemoSeedIds.ahmed,
      ...
      _membership(
        'membership-khaled',
        DemoSeedIds.technicalSupportQueue,
        DemoSeedIds.khaled,
        TeamMembershipRole.queueMember,
      ),
    ]);
    permissions.addAll(
      PermissionCode.values
          .where((e) => e != PermissionCode.unknown)
          .map((e) => Permission(id: 'permission-${e.code}', code: e)),
    );
    const matrix = <String, Set<PermissionCode>>{
      DemoSeedIds.roleEmployee: {
    ...
    for (final entry in matrix.entries) {
      for (final code in entry.value) {
        rolePermissions.add(
          RolePermission(
            id: '${entry.key}-permission-${code.code}',
            roleId: entry.key,
            permissionId: 'permission-${code.code}',
          ),
        );
      }
    }
    ...
    for (final code in [
      ...
      categories.add(
        TaskCategory(
          ...
          labelAr: code,
          labelEn: code,
          defaultPriorityId: DemoSeedIds.priorityNormal,
        ),
      );
    }
    confidentialityLevels.addAll(const [
      ...
      ConfidentialityLevel(
        id: DemoSeedIds.confidentialityInternal,
        code: 'internal',
        labelAr: 'داخلي',
        labelEn: 'Internal',
        level: 2,
    ...
    escalationRules.add(
      const EscalationRule(
        ...
        code: 'overdue',
        nameAr: 'تصعيد التأخير',
        nameEn: 'Overdue escalation',
      ),
    );
    notificationTemplates.add(
      const NotificationTemplate(
        id: 'notification-template-assigned',
        code: 'assigned',
        titleAr: 'مهمة جديدة',
        titleEn: 'New task',
        messageAr: 'تم تعيين مهمة',
        messageEn: 'A task was assigned',
      ),
    );
    taskTemplates.add(
      const TaskTemplate(
        id: 'task-template-standard',
        titleAr: 'مهمة قياسية',
        titleEn: 'Standard task',
    ...
    for (var i = 1; i <= 15; i++) {
      ...
      tasks.add(
        Task(
          ...
          creationSource: TaskCreationSource.manual,
          assignmentMode: i == 8
              ? AssignmentMode.individualCopies
              : (i == 11
                    ? AssignmentMode.teamQueue
                    : AssignmentMode.singleOwner),
          leadOwnerId: DemoSeedIds.ahmed,
          assignedTeamId: i == 11 || i == 12
              ? DemoSeedIds.technicalSupportQueue
              : DemoSeedIds.fieldOperations,
          batchId: i == 7 ? 'batch-policy-annual' : null,
          priorityId: i >= 10
              ? DemoSeedIds.priorityUrgent
              : DemoSeedIds.priorityNormal,
          categoryId: i == 11 ? 'category-support' : 'category-operations',
          status: i == 4 ? TaskStatus.blocked : TaskStatus.inProgress,
          confidentialityLevelId: DemoSeedIds.confidentialityInternal,
          visibilityType: TaskVisibilityType.assignedUsers,
          dueDate: now.add(Duration(days: i)),
          progressPercentage: i * 5,
          approvalRequired: i == 2 || i == 3,
          completionEvidenceRequired: i == 1,
          allowDecline: true,
          allowExtension: true,
          approvalStatus: i == 2
      ...
      assignments.add(
        TaskAssignment(
          id: 'assignment-$id',
          taskId: id,
          userId: i == 11 ? DemoSeedIds.khaled : DemoSeedIds.ahmed,
          assignmentRole: AssignmentRole.owner,
    ...
    for (var i = 1; i <= 11; i++) {
      ...
      tasks.add(
        Task(
          id: 'batch-copy-$i',
          taskNumber: 'BATCH-${i.toString().padLeft(2, '0')}',
          titleAr: source.titleAr,
          descriptionAr: source.descriptionAr,
          creatorId: source.creatorId,
          creatorRoleCode: source.creatorRoleCode,
          creationSource: TaskCreationSource.recurrence,
          assignmentMode: AssignmentMode.individualCopies,
          leadOwnerId: DemoSeedIds.ahmed,
          batchId: 'batch-policy-annual',
          priorityId: source.priorityId,
          categoryId: source.categoryId,
          status: TaskStatus.upcoming,
          confidentialityLevelId: source.confidentialityLevelId,
          visibilityType: source.visibilityType,
          dueDate: now.add(Duration(days: 30 + i)),
    ...
    checklistItems.addAll(const [
      ChecklistItem(
        id: 'check-01-a',
        taskId: 'scenario-01',
        titleAr: 'التحقق من الطلب',
        titleEn: 'Verify request',
        isMandatory: true,
        isCompleted: true,
      ),
      ChecklistItem(
    ...
    blockers.add(
      TaskBlocker(
        id: 'blocker-04',
        taskId: 'scenario-04',
        reportedBy: DemoSeedIds.ahmed,
        type: BlockerType.externalDependency,
        description: 'External finance dependency',
        startedAt: now.subtract(const Duration(days: 4)),
        status: BlockerStatus.active,
      ),
    );
    extensionRequests.add(
      DeadlineExtensionRequest(
        id: 'extension-09',
        taskId: 'scenario-09',
        requestedBy: DemoSeedIds.ahmed,
        currentDueDate: now.add(const Duration(days: 7)),
        requestedDueDate: now.add(const Duration(days: 14)),
    ...
    notifications.add(
      AppNotification(
        ...
        recipientId: DemoSeedIds.ahmed,
        type: NotificationType.taskAssigned,
        titleAr: 'مهمة جديدة',
        titleEn: 'Task assigned',
        messageAr: 'لديك مهمة',
        messageEn: 'You have a task',
        taskId: 'scenario-01',
        createdAt: now,
        isRead: false,
    ...
    syncOperations.addAll([
      SyncOperation(
        id: 'sync-pending',
        entityType: 'task',
        entityId: 'scenario-12',
      ...
      SyncOperation(
        id: 'sync-conflict',
        entityType: 'task',
        entityId: 'scenario-13',
        operationType: SyncOperationType.update,
        payloadJson: '{}',
        createdAt: now,
        status: SyncOperationStatus.conflict,
        conflictType: SyncConflictType.assignment,
      ),
    ]);
  }
  ...
  TeamMembership _membership(
    ...
    userId: user,
    membershipRole: role,
    joinedAt: now.subtract(const Duration(days: 365)),
  );
}

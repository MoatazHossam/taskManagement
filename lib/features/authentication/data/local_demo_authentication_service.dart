...
final class LocalDemoAuthenticationService implements AuthenticationService {
  LocalDemoAuthenticationService({
    required this.users,
  ...
  Future<AuthenticationResult> signInWithDemoCredentials({
    ...
    final profile = DemoAuthenticationConfig
        .credentialProfiles[username.trim().toLowerCase()];
    if (profile == null || password != DemoAuthenticationConfig.demoPassword) {
      await _event(
        AuditEventType.authenticationLoginFailed,
        DemoSeedIds.laila,
        'invalid_credentials',
      );
    ...
    if (isOffline()) {
      await _event(
        AuditEventType.authenticationOfflineDenied,
        seededUserIdForDemoProfile(
          demoProfiles.firstWhere((p) => p.id == profile),
        ),
        'new_login_offline',
      );
      return const AuthenticationResult.failure(
  ...
  @override
  Future<AuthenticationResult> signInWithDemoProfile(
    ...
    return _authenticate(
      demoProfileId,
      AuditEventType.authenticationProfileSelected,
    );
  }

  Future<AuthenticationResult> _authenticate(
    String profileId,
    ...
    try {
      ...
      if (matches.length != 1 || !demoProfileUserIds.containsKey(profileId)) {
        return const AuthenticationResult.failure(
          AuthenticationFailure.profileMappingMissing,
        );
      }
      ...
      if (user.status != UserStatus.active) {
        ...
        return const AuthenticationResult.failure(
          AuthenticationFailure.inactiveUser,
        );
      }
      ...
      if (repositoryRole != profile.role) {
        ...
        return const AuthenticationResult.failure(
          AuthenticationFailure.roleMismatch,
        );
      }
      final now = clock.now().toUtc();
      final session = AuthenticationSession(
        id: 'auth-${now.microsecondsSinceEpoch}',
        userId: user.id,
        ...
        offlineAccessExpiresAt: now.add(
          DemoAuthenticationConfig.offlineAccessDuration,
        ),
      );
      _active = session;
      ...
      return const AuthenticationResult.failure(
        AuthenticationFailure.databaseUnavailable,
      );
    }
  }
  ...
  Future<AuthenticationSession?> restoreSession({required bool offline}) async {
    try {
      ...
      final offlineExpires = DateTime.parse(
        values['${_prefix}offline_expires_at']!,
      ).toUtc();
      ...
      final session = AuthenticationSession(
        id: values['${_prefix}session_id']!,
        userId: userId,
        demoProfileId: profileId,
        systemRoleCode: _roleCode(_roleForId(user.roleId)),
        status: method == AuthenticationUnlockMethod.none
            ? AuthenticationSessionStatus.active
            : AuthenticationSessionStatus.locked,
        createdAt: DateTime.parse(values['${_prefix}created_at']!).toUtc(),
        lastAuthenticatedAt: DateTime.parse(
          values['${_prefix}last_authenticated_at']!,
        ).toUtc(),
        expiresAt: expires,
        offlineAccessExpiresAt: offlineExpires,
        isOfflineSession: offline,
        requiresUnlock: method != AuthenticationUnlockMethod.none,
        unlockMethod: method,
      );
      _active = session;
      await _event(
        offline
            ? AuditEventType.authenticationOfflineUsed
            : AuditEventType.authenticationSessionRestored,
        userId,
        'session_restored',
        session.id,
      );
  ...
  Future<AuthenticationResult> unlockWithPin(String pin) async {
    ...
    if (_pinAttempts >= DemoAuthenticationConfig.maximumPinAttempts) {
      return const AuthenticationResult.failure(
        AuthenticationFailure.tooManyPinAttempts,
      );
    }
    if (pin != DemoAuthenticationConfig.demoPin) {
      _pinAttempts++;
      await _event(
        AuditEventType.authenticationPinFailed,
        session.userId,
        'pin_failed',
        session.id,
      );
      return AuthenticationResult.failure(
        _pinAttempts >= DemoAuthenticationConfig.maximumPinAttempts
            ? AuthenticationFailure.tooManyPinAttempts
    ...
    _active = session.copyWith(
      ...
      lastAuthenticatedAt: clock.now().toUtc(),
    );
    await _event(
      AuditEventType.authenticationPinSucceeded,
      session.userId,
      'pin_succeeded',
      session.id,
    );
  ...
  @override
  Future<AuthenticationResult> unlockWithBiometrics({
    ...
    if (!simulateSuccess) {
      await _event(
        AuditEventType.authenticationBiometricFailed,
        session.userId,
        'simulation_failed',
        session.id,
      );
    ...
    await _event(
      AuditEventType.authenticationBiometricSucceeded,
      session.userId,
  ...
  Future<void> expireSession() async {
    if (_active != null)
      await _event(
        AuditEventType.authenticationSessionExpired,
        _active!.userId,
        'session_expired',
        _active!.id,
      );
  ...
  Future<void> signOut() async {
    if (_active != null)
      await _event(
        AuditEventType.authenticationLogout,
        _active!.userId,
        'logout',
        _active!.id,
      );
  ...
  Future<void> _persist(AuthenticationSession session) async {
    final values = <String, String>{
      ...
      'last_profile_id': session.demoProfileId,
    ...
    for (final entry in values.entries)
      await settings.saveSetting('$_prefix${entry.key}', entry.value);
  }

...
class _SplashPageState extends ConsumerState<SplashPage> {
  ...
  void initState() {
    ...
    Future.microtask(() async {
      ref.read(demoDataStoreProvider);
      await ref
...
class SegmentationLanguageSelector extends StatelessWidget {
  const SegmentationLanguageSelector({
    super.key,
...
class DemoLoginPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DemoLoginPage> createState() => _DemoLoginPageState();
}
...
class _DemoLoginPageState extends ConsumerState<DemoLoginPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final expired = session.status == AuthenticationStatus.expired;
    return Scaffold(
      appBar: AppBar(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                final form = _LoginForm(
                  ...
                  expired: expired,
                  loading:
                      session.status == AuthenticationStatus.authenticating,
                  ...
                  onLogin: () {
                    if (formKey.currentState!.validate()) {
                      ref
                          .read(sessionProvider.notifier)
                          .signInCredentials(username.text, password.text);
                    }
                  },
                );
                final profiles = _ProfilePicker(
                  onSelected: (profile) =>
                      ref.read(sessionProvider.notifier).signInProfile(profile),
                );
                  ...
                  child: wide
                      ? Row(
                          ...
                          children: [
                            ...
                            Expanded(flex: 6, child: profiles),
                          ],
                        )
                      : Column(
                          children: [
...
class _LoginForm extends StatelessWidget {
  ...
  final bool expired, loading, obscure;
        ...
        child: Column(
          ...
          children: [
            AppPageHeader(
              ...
              subtitle: context.l10n.authenticationSimulated,
            ),
            if (expired) ...[
              const SizedBox(height: AppSpacing.medium),
              AppBadge(
                label: context.l10n.sessionExpired,
            ...
            if (error != null) ...[
              const SizedBox(height: AppSpacing.medium),
              Text(
                _authenticationError(context, error!),
            ...
            TextFormField(
              ...
              controller: username,
              validator: (value) =>
                  (value ?? '').trim().isEmpty ? context.l10n.username : null,
            ...
            TextFormField(
              ...
              controller: password,
              validator: (value) =>
                  (value ?? '').isEmpty ? context.l10n.password : null,
              obscureText: obscure,
              decoration: InputDecoration(
            ...
            const SizedBox(height: AppSpacing.medium),
            Text(
              context.l10n.demoCredentialsHelper,
...
String _authenticationError(
  ...
  AuthenticationFailure.invalidCredentials => context.l10n.invalidCredentials,
  AuthenticationFailure.inactiveUser => context.l10n.userInactive,
  AuthenticationFailure.pinInvalid => context.l10n.incorrectPin,
  AuthenticationFailure.tooManyPinAttempts => context.l10n.tooManyAttempts,
  AuthenticationFailure.offlineAccessExpired =>
    context.l10n.offlineAccessExpired,
  AuthenticationFailure.roleMismatch => context.l10n.permissionError,
  AuthenticationFailure.databaseUnavailable => context.l10n.storageError,
...
class _PinUnlockPageState extends ConsumerState<PinUnlockPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(context.l10n.unlockApplication)),
      body: SafeArea(
        child: Center(
          child: SingleChildScrollView(
            padding: const EdgeInsets.all(AppSpacing.large),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (state.failure != null)
                        Text(
                          _authenticationError(context, state.failure!),
                          style: TextStyle(
                      ...
                      FilledButton(
                        onPressed: () => ref
                            .read(sessionProvider.notifier)
                            .unlockPin(pin.text),
                        child: Text(context.l10n.unlockApplication),
                      ),
                      TextButton.icon(
                        icon: const Icon(Icons.fingerprint),
                        label: Text(context.l10n.useSimulatedBiometrics),
                        onPressed: () => showDialog<void>(
                          context: context,
                          builder: (context) => AlertDialog(
                            icon: const Icon(Icons.fingerprint),
                            title: Text(context.l10n.useSimulatedBiometrics),
                            content: Text(
                              context.l10n.biometricSimulationNotice,
                            ),
                            actions: [
                              TextButton(
                                onPressed: () {
                                ...
                                child: Text(context.l10n.simulateFailure),
                              ),
                              FilledButton(
                                onPressed: () {
                                  Navigator.pop(context);
                                ...
                                child: Text(context.l10n.simulateSuccess),
                              ),
                            ],
                          ),
                        ),
...
class _ProfilePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Text(
        context.l10n.selectDemoProfile,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${roleLabel(context, profile.role)} • ${profile.localizedDepartment(language)}',
                      style: Theme.of(context).textTheme.bodySmall,
                    ),
                  ],
                ),

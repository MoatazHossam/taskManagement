...
void main(){late AppDatabase db;late DemoDataService seed;setUp((){db=AppDatabase.forTesting(NativeDatabase.memory());seed=DemoDataService(db,FixedAppClock(DateTime.utc(2026,1,15,10)));});tearDown(() async=>db.close());
 ...
 test('unique task number, foreign key and progress constraints are enforced',() async {await seed.ensureSeeded();await expectLater(db.customStatement("UPDATE tasks SET task_number='SCN-01' WHERE id='scenario-02'"),throwsA(anything));await expectLater(db.customStatement("UPDATE tasks SET category_id='missing' WHERE id='scenario-01'"),throwsA(anything));await expectLater(db.customStatement("UPDATE tasks SET progress_percentage=101 WHERE id='scenario-01'"),throwsA(anything));}); }

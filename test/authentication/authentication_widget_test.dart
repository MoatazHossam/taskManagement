...
void main() {
  for (final locale in const [Locale('ar'), Locale('en')]) {
    testWidgets('login direction and responsive profile cards ${locale.languageCode}',
        (tester) async {
  ...
  testWidgets('credential validation and visibility control are semantic',
    ...
    final before = tester.widget<TextFormField>(find.byKey(const Key('password-input')));
    expect(before.obscureText, isTrue);
    await tester.tap(find.byKey(const Key('password-visibility')));
    await tester.pump();
    final after = tester.widget<TextFormField>(find.byKey(const Key('password-input')));

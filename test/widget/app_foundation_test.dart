...
void main() {
  testWidgets('application starts and language direction changes', (
    tester,
  ) async {
    await tester.pumpWidget(const ProviderScope(child: TaskManagementApp()));
    await tester.pumpAndSettle();
    expect(find.byType(LanguageSelectionPage), findsOneWidget);
    final e = ProviderScope.containerOf(
      tester.element(find.byType(LanguageSelectionPage)),
    );
    e.read(localeProvider.notifier).state = const Locale('ar');
    await tester.pump();
    expect(
      Directionality.of(tester.element(find.byType(LanguageSelectionPage))),
      TextDirection.rtl,
    );
    e.read(localeProvider.notifier).state = const Locale('en');
    await tester.pump();
    expect(
      Directionality.of(tester.element(find.byType(LanguageSelectionPage))),
      TextDirection.ltr,
  ...
  for (final entry in <DemoUserRole, String>{
    DemoUserRole.employee: 'employee',
    DemoUserRole.manager: 'manager',
    DemoUserRole.seniorManagement: 'senior',
    DemoUserRole.administrator: 'administrator',
  }.entries) {
    testWidgets('${entry.key.name} profile opens its navigation', (
      tester,
    ) async {
      final profile = demoProfiles.firstWhere((p) => p.id == entry.value);
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            sessionProvider.overrideWith((ref) {
              final c = SessionController();
              c.authenticate(profile);
              return c;
            }),
          ],
  ...
  testWidgets('offline and unstable banners are explicit', (tester) async {
    ...
    await tester.pumpWidget(
      UncontrolledProviderScope(
        ...
        child: const _LocalizedTestApp(
          child: Scaffold(body: ConnectivityStatusBanner()),
        ),
      ),
    );
  ...
  testWidgets('placeholder is honest and survives large text', (tester) async {
    await tester.pumpWidget(
  ...
  testWidgets('employee dashboard is compact at small-phone width', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      _profileApp(
        ...
        child: const RoleHomePage(title: 'Home'),
      ),
    );
    await tester.pump();
    expect(find.byType(AppMetricCard), findsNWidgets(4));
    expect(find.byType(AppTaskPreviewTile), findsWidgets);
    expect(tester.takeException(), isNull);
  });
  testWidgets('employee dashboard supports large-phone width and large text', (
    tester,
  ) async {
  ...
  for (final locale in const [Locale('ar'), Locale('en')]) {
    testWidgets(
      'profile has one identity treatment in ${locale.languageCode}',
      (tester) async {
        await tester.pumpWidget(
          _profileApp(
            locale: locale,
            child: ProfilePage(openGallery: () {}),
          ),
        );
        await tester.pump();
        expect(find.byType(AppAvatar), findsOneWidget);
        expect(find.byType(ConnectivitySelector), findsOneWidget);
        expect(tester.takeException(), isNull);
      },
    );
  }
  testWidgets('dark mode surfaces and long Arabic connectivity option render', (
    tester,
  ) async {
    await tester.pumpWidget(
      _profileApp(
    ...
    await tester.pump();
    expect(find.text('اتصال غير مستقر'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
  testWidgets('connectivity option selection is exposed', (tester) async {
    ...
    await tester.pumpWidget(
      _LocalizedTestApp(
        child: StatefulBuilder(
          builder: (context, setState) => ConnectivitySelector(
            value: value,
    ...
    await tester.tap(
      find.byKey(const ValueKey(SimulatedConnectivityStatus.unstable)),
    );
    await tester.pump();
    expect(value, SimulatedConnectivityStatus.unstable);
  });
  testWidgets('language control changes selection', (tester) async {
    ...
    await tester.pumpWidget(
      _LocalizedTestApp(
        child: StatefulBuilder(
          builder: (context, setState) => SegmentationLanguageSelector(
            locale: locale,
            onChanged: (next) => setState(() => locale = next),
          ),
        ),
      ),
    );
  ...
  testWidgets('primary scroll view reserves phone navigation clearance', (
    tester,
  ) async {
    await tester.pumpWidget(
      const _LocalizedTestApp(
        child: AppPrimaryScrollView(children: [SizedBox(height: 900)]),
      ),
    );
    final list = tester.widget<ListView>(
      find.byKey(const Key('primary-scroll-view')),
    );
...
Widget _profileApp({
  ...
  final profile = demoProfiles.firstWhere(
    (profile) => profile.id == 'employee',
  );
  return ProviderScope(
    overrides: [
      sessionProvider.overrideWith((ref) {
        final controller = SessionController();
        controller.authenticate(profile);
        return controller;
      }),
    ],

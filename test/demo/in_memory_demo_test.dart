...
void main() {
  ...
  setUp(() {
    ...
    auth = LocalDemoAuthenticationService(users: users, settings: settings, audit: audit, clock: clock, isOffline: () => false);
  });
  ...
  test('all five profiles and credential aliases authenticate', () async {
    ...
    for (final alias in ['employee', 'manager', 'executive', 'admin', 'support']) {
      expect((await auth.signInWithDemoCredentials(username: alias, password: 'demo123')).isSuccess, isTrue);
      await auth.signOut();
    }
    expect((await auth.signInWithDemoCredentials(username: 'employee', password: 'wrong')).isSuccess, isFalse);
  });

  test('PIN, biometric, offline, logout, switching, and audit redaction', () async {
    expect((await auth.signInWithDemoProfile(demoProfiles.first.id)).isSuccess, isTrue);
    expect((await auth.unlockWithPin('0000')).isSuccess, isFalse);
    expect((await auth.unlockWithPin('1234')).isSuccess, isTrue);
    expect((await auth.unlockWithBiometrics(simulateSuccess: false)).isSuccess, isFalse);
    ...
    await auth.signOut();
    expect(await settings.getSetting('authentication.active_user_id'), isNull);
    final serialized = (await audit.getAuditEvents()).map((e) => '${e.reason}${e.entityId}').join();
    expect(serialized, isNot(contains('1234')));
    expect(serialized, isNot(contains('0000')));
    final offline = LocalDemoAuthenticationService(users: users, settings: settings, audit: audit, clock: clock, isOffline: () => true);
    expect((await offline.signInWithDemoProfile(demoProfiles.first.id)).isSuccess, isFalse);
  });
  ...
  test('repositories filter and expose immutable snapshots', () async {
    ...
    expect(await users.getUsersByTeam(DemoSeedIds.technicalSupportQueue), hasLength(1));
    final organization = InMemoryOrganizationRepository(store);
    expect(await organization.getTeamsByDepartment(DemoSeedIds.operations), hasLength(2));
    expect(() => (store.users.toList(growable: false)).add(store.users.first), throwsUnsupportedError);
  });

  test('all scenarios and their typed references are internally valid', () async {
    final scenarioIds = {for (var i = 1; i <= 15; i++) DemoSeedIds.scenario(i)};
    ...
    expect(await InMemorySyncRepository(store).getConflictingOperations(), hasLength(1));
    expect(store.roles.map((e) => e.code), containsAll(SystemRoleCode.values.where((e) => e != SystemRoleCode.unknown)));
  });
}
